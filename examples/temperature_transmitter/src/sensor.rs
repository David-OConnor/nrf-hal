...
pub fn set_emissivity(emis: f32, twim: &mut Twim<TWIM0>) -> Result<(), SensorError> {
    ...
    let pec = smbus_pec::pec(&[SENSOR_ADDR << 1, EMIS_EEPROM_CMD, 0, 0]); // todo TS
...
pub fn sleep(twim: &mut Twim<TWIM0>, scl: &mut Pin) {
    ...
    // Set SCL low to reduce parasitic power loss in sleep mode, as direct by the
    // MLX90614 datasheet.
    ...
    scl.dir(Dir::Output);
    scl.drive(Drive::S0S1);
    scl.pull(Pull::Disabled);
    scl.set_low();
}

/// Wake from sleep mode. See datasheet, section 8.4.8.
/// "SCL pin high and then PWM/SDA pin low for at least t_(DDQ) > 33ms"
pub fn wake(_twim: &mut Twim<TWIM0>, scl: &mut Pin, sda: &mut Pin, timer: &mut Timer<TIMER1>) {
    // Set SCL and SDA to output pins so we can set them manually.
    // todo: We probably don't need to set SCL to be an normal output pin here, since
    ...
    // Note: "On-chip IIR filter is skipped for the very first measurement"
    // So, we take a dummy measurement here.
    // read_temp(i2c);
}
